...
export const updateLeaseAgreement = async (id, leaseAgreementData) => {
  try {
    if (!leaseAgreementData.leaseAgreementDocument) {
      throw new Error('Lease agreement document is required');
    }
    const response = await api.put(`${API_URL}/leaseAgreement/update/${id}`,leaseAgreementData );
